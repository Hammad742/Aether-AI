...
const AuthenticatedApp = () => {
    ...
    const handleSubmit = async (event, textOverride = null) => {
        ...
        if (loading) return
        if (!hasText && !hasFiles && (!isVisionModel || !hasImage)) return
        setValidationError('')
        if (!userApiKey) {
        ...
        try {
            let activeSessionId = currentSessionId || createNewChat();
            if (activePrompt.trim().toLowerCase().startsWith('/imagine ')) {
                await processImageGeneration(activePrompt, activeSessionId);
                return;
            }
            const parts = []
            ...
            if (hasText || fallbackText) parts.push({ type: 'text', text: hasText ? activePrompt.trim() : fallbackText })
            ...
            const displayUserMessage = { role: 'user', content: hasText ? activePrompt.trim() : (fallbackText || (hasFiles ? "Sent attachments" : "Sent an attachment")) }
            ...
            const newMessageForApi = { role: 'user', content: parts.length > 0 ? parts : activePrompt.trim() };
            ...
            if (isWebSearchEnabled) {
                ...
                const searchContext = await performWebSearch(activePrompt);
                setValidationError('')
    ...
    const handleVoiceSubmit = async (voiceText) => await handleSubmit(null, voiceText);
